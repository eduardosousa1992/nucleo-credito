...
# ══════════════════════════════════════════════════════════════════════════
# CSS PREMIUM (Estilização da Topbar e Ocultação de Elementos Nativos)
# ══════════════════════════════════════════════════════════════════════════
st.markdown(f"""
<style>
...
/* Injeção de estilo para os botões nativos dentro da barra de navegação */
div[data-testid="stHorizontalBlock"].topbar-nav-container {{
  background: transparent !important;
  gap: 4px !important;
  flex: 1 !important;
  align-items: center !important;
}}
div[data-testid="stHorizontalBlock"].topbar-nav-container button {{
  background: transparent !important;
...
div[data-testid="stHorizontalBlock"].topbar-nav-container button:hover {{
  background: rgba(255,255,255,0.12) !important;
  color: white !important;
}}
div[data-testid="stHorizontalBlock"].topbar-nav-container div[data-testid="element-container"] {{
  width: auto !important;
}}

.topbar-right {{ display: flex; align-items: center; gap: 12px; margin-left: auto; flex-shrink: 0; }}
.topbar-user {{ color: rgba(255,255,255,0.85); font-size: 13px; font-weight: 600; background: rgba(255,255,255,0.1); padding: 5px 12px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.15); }}

.main-content {{ margin-top: 80px; padding: 0 8px; }}
...
.kpi {{ background: white; border-radius: 12px; padding: 16px 14px; box-shadow: 0 2px 6px rgba(0,0,0,0.05); border-top: 3px solid {GREEN}; text-align: center; }}
...
.chart-card {{ background: white; border-radius: 14px; padding: 18px 20px; box-shadow: 0 2px 6px rgba(0,0,0,0.05); margin-bottom: 14px; }}
.ct {{ font-size: 12px; font-weight: 700; color: {NAVY}; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid #F0F0F0; }}
...
.client-card {{ background: white; border-radius: 12px; padding: 14px 18px; margin-bottom: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.04); border-left: 4px solid #E0E0E0; }}
.cc-opp {{ border-left-color: {GREEN}; }}
.cc-warn {{ border-left-color: {YELLOW}; }}
.cc-red {{ border-left-color: {RED}; }}

.login-wrap {{ max-width: 360px; margin: 80px auto; background: white; border-radius: 18px; padding: 36px 32px; box-shadow: 0 8px 40px rgba(0,0,0,0.12); text-align: center; }}
</style>
""", unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════════════
# GESTÃO DE SECRETS E VARIÁVEIS DE AMBIENTE (Fallback Estruturado)
# ══════════════════════════════════════════════════════════════════════════
def gs(key, default=""):
    try:
...
# ══════════════════════════════════════════════════════════════════════════
# CONEXÃO SUPABASE COM STRATEGY CACHE
# ══════════════════════════════════════════════════════════════════════════
@st.cache_resource
def get_sb():
    ...
    try:
        ...
        st.error(f"Conexão com o Supabase falhou: {e}")
        return None

sb = get_sb()

# ══════════════════════════════════════════════════════════════════════════
# INFRAESTRUTURA DE SEGURANÇA E CRIPTOGRAFIA (LGPD Compliance)
# ══════════════════════════════════════════════════════════════════════════
def _f():
    k = gs("ENCRYPT_KEY", "")
    if k:
...
# ══════════════════════════════════════════════════════════════════════════
# ALGORITMO DE SCORE DE PROPENSÃO DE COMPRA
# ══════════════════════════════════════════════════════════════════════════
def score(row):
    s = 0
    m = row.get("margem", 0)
...
# ══════════════════════════════════════════════════════════════════════════
# INTEGRAÇÃO DE SESSÃO TRANSMISSION (BREVO API V3)
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# CAMADA DE PERSISTÊNCIA E LOGÍSTICA DE DADOS (DML & DQL)
# ══════════════════════════════════════════════════════════════════════════
def load_clientes():
    ...
    try:
        ...
        st.error(f"Erro na carga de Clientes: {e}")
        return pd.DataFrame()
...
# ══════════════════════════════════════════════════════════════════════════
# ENGINE DE AUTENTICAÇÃO E PERMISSÕES (HMAC / SHA-256)
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# FORMATADORES E CONTROLE DE ESTILO EM GRÁFICOS
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# GERENCIAMENTO DE ESTADO DA SESSÃO LOCAL (Session Memory)
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# INTERFACE GLOBAL: DESIGN DA TOPBAR PREMIUM E CORREÇÃO DO FLUXO DE EVENTOS
# ══════════════════════════════════════════════════════════════════════════
PAGES = ["Dashboard", "Clientes", "Leads", "Contratos", "Simulador", "Alertas", "Agenda", "Email Marketing", "Metas"]

# Renderiza a estrutura visual básica da Topbar via HTML
st.markdown(f"""
<div class="topbar">
  ...
  <div id="nav-mount-point"></div>
</div>
""", unsafe_allow_html=True)

# Criação dos botões estáveis e responsivos do Streamlit acoplados na Topbar
# Essa arquitetura corrige o erro de recarga de página e preserva a sessão intacta
with st.container():
    st.write('<div style="position:fixed; top:12px; left:230px; width:700px; z-index:100000;">', unsafe_allow_html=True)
    nav_cols = st.columns(len(PAGES))
    for idx, page_name in enumerate(PAGES):
        with nav_cols[idx]:
            # Adiciona classe ativa dinamicamente via Markdown caso seja a página selecionada
            is_selected = st.session_state.page == page_name
            label_decorado = f"{page_name}"
            if st.button(label_decorado, key=f"btn_nav_{page_name}", use_container_width=True):
                st.session_state.page = page_name
                st.rerun()
    st.write('</div>', unsafe_allow_html=True)

# Renderiza a área direita com as informações de login do usuário logado
primeiro_nome = st.session_state.uname.split()[0]
st.markdown(f"""
<div style="position:fixed; top:12px; right:24px; z-index:100000;">
  <span class="topbar-user">👤 {primeiro_nome}</span>
</div>
""", unsafe_allow_html=True)

# Gerenciamento de navegação via parâmetros na Query URL
qp = st.query_params
if "page" in qp and qp["page"] in PAGES:
    st.session_state.page = qp["page"]

# Fallback lateral escondido apenas para logoff de emergência
with st.sidebar:
...
# ═══ ALERTAS ═══
    ...
    if len(opp) > 0:
        ...
        for _, row in opp.iterrows():
            ...
            texto_badge_m = badge("🔥 Máx", "y") if h else badge("Oportunidade", "g")
            st.markdown(f'<div style="background:{bg};border:1px solid {bc};border-radius:12px;padding:14px 18px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:flex-start"><div><div style="font-size:14px;font-weight:800;color:{NAVY}">{sn}</div><div style="font-size:11px;color:#888;margin:3px 0 6px">{row["tel_d"]} · {row["interesse"]} · {row["canal"]}</div><div>{texto_badge_m} {badge(f"Score {sc}%", "n")} {badge(st2, "b")}</div></div><div style="text-align:right"><div style="font-size:11px;color:#888">Margem disponível</div><div style="font-size:22px;font-weight:800;color:{GREEN}">{fmt(row["margem"])}</div></div></div>', unsafe_allow_html=True)
    if len(bx) > 0:
