...
.login-wrap {{ max-width: 360px; margin: 100px auto; background: white; border-radius: 18px; padding: 36px 32px; box-shadow: 0 10px 40px rgba(0,0,0,0.06); text-align: center; }}
</style>
...
# ── MODELO MATEMÁTICO: CALENDÁRIO INSS 2026 ────────────────────────────────
INSS = {
    ...
    if not df.empty:
        ...
        if len(urg) > 0:
            ...
            for _, r in urg.iterrows():
                ...
                st.markdown(f"""<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid #F1F5F9">
                  <div><span style="font-weight:700;color:{NAVY};font-size:13px">{r['nome'].split()[0]} {r['nome'].split()[1] if len(r['nome'].split()) > 1 else ''}</span>
                  <span style="font-size:11px;color:#64748B;margin-left:8px">{r['tel_d']}</span></div>
...
# ═══ CLIENTES ═══
    ...
    with st.form("fc", clear_on_submit=True):
        ...
        if st.form_submit_button("✅ Salvar Ficha Cadastral", use_container_width=True):
            if not nome or not ben: st.error("Os campos Nome e Benefício são mandatórios.")
            else:
        ...
        for _, row in dff.iterrows():
            ...
            with ca:
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:15px;">
                    <div style="padding:14px; background:#F8FAFC; border-radius:8px; border:1px solid #E2E8F0;"><span style="font-size:11px; color:#64748B; font-weight:700;">BENEFÍCIO BASE</span><br><strong style="font-size:16px; color:{NAVY};">{fmt(row['beneficio'])}</strong></div>
                    <div style="padding:14px; background:#F8FAFC; border-radius:8px; border:1px solid #E2E8F0;"><span style="font-size:11px; color:#64748B; font-weight:700;">MARGEM LÍQUIDA DISP.</span><br><strong style="font-size:16px; color:{GREEN if m>300 else RED};">{fmt(m)}</strong></div>
                    <div style="padding:14px; background:#F8FAFC; border-radius:8px; border:1px solid #E2E8F0;"><span style="font-size:11px; color:#64748B; font-weight:700;">PRÓXIMO INSS</span><br><strong style="font-size:16px; color:{NAVY};">{row['prox'].strftime('%d/%m/%Y') if row['prox'] else '—'}</strong></div>
                    <div style="padding:14px; background:#F8FAFC; border-radius:8px; border:1px solid #E2E8F0;"><span style="font-size:11px; color:#64748B; font-weight:700;">COMPROMETIMENTO</span><br><strong style="font-size:16px; color:{NAVY};">{row['pct']:.0f}%</strong></div>
                </div>
                """, unsafe_allow_html=True)
            with cb:
                ...
                with st.form(f"fu_form_{row['id']}"):
                    ...
                    mfu = st.text_input("Motivo", key=f"mfu_{row['id']}", placeholder="Ex: Cobrar extrato de empréstimos")
            ...
            if not hist.empty:
                st.markdown("<p style='font-size:12px; font-weight:700; color:#334155; margin:5px 0;'>📋 Historial Recente de Atendimentos:</p>", unsafe_allow_html=True)
                for _, h in hist.head(2).iterrows():
...
# ═══ LEADS ═══
    ...
    if not dfl.empty:
        ...
        for idx, s in enumerate(STATUS):
            g = dfl[dfl["status"] == s]
            with cols[idx]:
                st.markdown(f'<div class="content-card" style="background:#F8FAFC !important; border-top: 3px solid {CORES2[s]} !important;"><h5 style="color:{CORES2[s]}; margin:0 0 15px 0;">{s} ({len(g)})</h5>', unsafe_allow_html=True)
        ...
        st.info("Pipeline de prospecção livre no momento.")

# ═══ CONTRATOS ═══
elif pg == "Contratos":
    st.markdown('<div class="page-header"><h2>📄 Emissão e Gestão de Contratos</h2><p>Controle financeiro de comissionamento e propostas fechadas</p></div>', unsafe_allow_html=True)
    df_cli = load_clientes(); dfc = load_contratos()
    
    if not dfc.empty:
        tv = dfc["valor"].sum(); tc2 = (tv * 0.03)
        c1, c2, c3 = st.columns(3)
        with c1: st.markdown(kpi("Volume Comercial Total", fmt(tv), "", "n"), unsafe_allow_html=True)
        with c2: st.markdown(kpi("Comissão Líquida Estimada", fmt(tc2), "3% sobre volume", "g"), unsafe_allow_html=True)
        with c3: st.markdown(kpi("Propostas Emitidas", str(len(dfc)), "Base ativa", "g"), unsafe_allow_html=True)
        st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
        
    st.markdown('<div class="content-card"><h4>➕ Formalizar Novo Contrato</h4>', unsafe_allow_html=True)
    if df_cli.empty: 
        st.warning("Cadastre um cliente antes de emitir propostas financeiras.")
    else:
        with st.form("fct_form", clear_on_submit=True):
    ...
    if not dfc.empty:
        ...
        st.markdown('<div class="chart-card"><h5>📋 Relação de Contratos Ativos</h5>', unsafe_allow_html=True)
        st.dataframe(dfc[["cliente_nome", "banco", "valor", "parcelas_total", "parcela", "comissao", "data_inicio"]].rename(columns={"cliente_nome": "Cliente", "banco": "Banco", "valor": "Valor Total", "parcelas_total": "Parcelas", "parcela": "Parcela/mês", "comissao": "Comissão", "data_inicio": "Data Emissão"}), use_container_width=True, hide_index=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
# ═══ ALERTAS ═══
    ...
    if df.empty: 
        ...
        with c3: st.markdown(kpi("Urgente (Pagamento INSS)", str(len(ph)), "Gatilho de contato imediato", "r"), unsafe_allow_html=True)
        st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
        
        if len(opp) > 0:
            st.markdown(f"<h5 style='color:{GREEN}; font-weight:700;'>🎯 Carteira Ativa Qualificada para Abordagem</h5>", unsafe_allow_html=True)
            for _, row in opp.iterrows():
                st.markdown(f"""
                <div class="content-card" style="border-left: 4px solid {GREEN} !important; display: flex; justify-content: space-between; align-items: center;">
                    <div>
...
# ═══ AGENDA ═══
    ...
    if not dfa2.empty:
        ...
        for titulo, grupo, col_color in [("🚨 Retornos em Atraso Vencidos", venc, RED), ("📌 Agenda do Dia de Hoje", hj, GREEN), ("📆 Retornos Futuros Programados", prx, NAVY)]:
            if len(grupo) == 0: continue
            st.markdown(f"<h5 style='color:{col_color}; font-weight:700; margin-top:15px;'>{titulo} ({len(grupo)})</h5>", unsafe_allow_html=True)
            for _, r in grupo.iterrows():
                ca2, cb2 = st.columns([5, 1])
                with ca2:
                    st.markdown(f"""
                    <div class="content-card" style="border-left:3px solid {col_color} !important; padding:14px !important;">
...
# ═══ EMAIL MARKETING ═══
    ...
    st.markdown('<div class="page-header"><h2>📧 Gestão de Campanhas e Régua de Relacionamento</h2><p>Mailing de e-mail marketing estruturado via Brevo Engine</p></div>', unsafe_allow_html=True)
    df = load_clientes()
    dce = df[df["email"].notna() & (df["email"] != "")] if not df.empty else pd.DataFrame()
    c1, c2, c3 = st.columns(3)
    with c1: st.markdown(kpi("Mailing Ativo", len(dce), "Clientes com e-mail", "n"), unsafe_allow_html=True)
    with c2: st.markdown(kpi("Cota de Envio Diária", "300 e-mails", "Plano Brevo Free", "g"), unsafe_allow_html=True)
    with c3: st.markdown(kpi("Custo de Infraestrutura", "R$ 0,00", "Sem custos ativos", "g"), unsafe_allow_html=True)
    ...
    with t1:
        ...
        if dce.empty: st.warning("Nenhum cliente com e-mail cadastrado localizado na base.")
            ...
            if st.button("📧 Disparar Régua de INSS", use_container_width=True):
                env = 0
                for _, row in dce[dce["nome"].isin(sel)].iterrows():
                    pg3, dias3 = prox_pg(row["cpf_raw"])
                    html = f'<div style="font-family:sans-serif;max-width:500px;margin:0 auto"><div style="background:{NAVY};padding:20px;border-radius:10px 10px 0 0"><h2 style="color:white;margin:0">⚛ {SENDER_NAME}</h2></div><div style="background:white;padding:24px"><p>Olá <b>{row["nome"].split()[0]}</b>! Seu próximo INSS: <b style="color:{GREEN}">{pg3.strftime("%d/%m/%Y") if pg3 else "Em breve"}</b> {f"(em {dias3} dia(s))" if dias3 is not None else ""}.</p></div></div>'
                    if send_email(row["email"], row["nome"], f"Calendário de Pagamentos INSS — {SENDER_NAME}", html): env += 1
                st.success(f"✅ Régua finalizada: {env} e-mail(s) despachado(s) com sucesso!")
    with t2:
        DICAS = [
            ("Proteção de Benefício INSS", "Nunca forneça seus dados bancários ou senhas por telefone. Golpistas usam engenharia social simulando o INSS."),
            ("Garantia de Portabilidade", "Se as taxas de juros dos seus empréstimos antigos superam 2,0% a.m., você tem direito de migrar sua dívida gratuitamente.")
        ]
        ds = st.selectbox("Selecione o Modelo de Campanha Informacional", [d[0] for d in DICAS])
        db = next(d[1] for d in DICAS if d[0] == ds)
        st.info(db)
        if not dce.empty:
            sel2 = st.multiselect("Clientes Destinatários", dce["nome"].tolist(), default=dce["nome"].tolist(), key="ds2")
            if st.button("📧 Enviar Informativo", use_container_width=True):
                env2 = 0
                for _, row in dce[dce["nome"].isin(sel2)].iterrows():
                    html = f'<div style="font-family:sans-serif;max-width:500px;margin:0 auto"><div style="background:{NAVY};padding:20px;border-radius:10px 10px 0 0"><h2 style="color:white;margin:0">⚛ {SENDER_NAME}</h2></div><div style="background:white;padding:24px"><p>Olá <b>{row["nome"].split()[0]}</b>!</p><div style="border-left:4px solid {GREEN};padding:12px 16px;background:#F8FFFE"><b style="color:{NAVY}">{ds}</b><p style="color:#555;margin-top:8px">{db}</p></div></div></div>'
                    if send_email(row["email"], row["nome"], f"💡 Informativo: {ds}", html): env2 += 1
                st.success(f"✅ Campanha concluída: {env2} e-mails disparados!")
    with t3:
        ca3 = st.text_input("Assunto do E-mail"); ct3 = st.text_input("Título Interno (Header)"); cc3 = st.text_area("Corpo do E-mail (HTML permitido)", height=120)
        if not dce.empty:
            sel3 = st.multiselect("Destinatários", dce["nome"].tolist(), key="ds3")
            if st.button("📣 Disparar Campanha Customizada", use_container_width=True):
                if not ca3 or not cc3: st.error("Os campos Assunto e Corpo da Mensagem são obrigatórios.")
                else:
                    env3 = 0
                    for _, row in dce[dce["nome"].isin(sel3)].iterrows():
                        html = f'<div style="font-family:sans-serif;max-width:500px;margin:0 auto"><div style="background:{NAVY};padding:20px;border-radius:10px 10px 0 0"><h2 style="color:white;margin:0">⚛ {SENDER_NAME}</h2></div><div style="background:white;padding:24px"><p>Olá <b>{row["nome"].split()[0]}</b>!</p><h3 style="color:{NAVY}">{ct3}</h3><p style="color:#555">{cc3}</p></div></div>'
                        if send_email(row["email"], row["nome"], ca3, html): env3 += 1
                    st.success(f"✅ Disparos efetuados: {env3} e-mails transmitidos!")

# ═══ METAS ═══
elif pg == "Metas":
    st.markdown('<div class="page-header"><h2>🎯 Planejamento de Metas Comerciais</h2><p>Métricas de performance operacional e monitoramento de comissões</p></div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    ...
    for lbl, atual, meta, cor2 in [("Contratos Consolidados", ctm, mc2, GREEN), ("Mailing de Leads Expandido", lm, ml, NAVY), ("Receita Bruta Conquistada", com, mcm, YELLOW)]:
        ...
        <div class="content-card">
          ...
          <div style="font-size:11px; color:{GREEN if pct3>=100 else '#64748B'}; font-weight:600; margin-top:6px;">
            {'🎉 Meta operacional atingida!' if pct3>=100 else f'Faltam {mf if "R$" in lbl else str(int(meta-atual))} para bater o teto estipulado'}
          </div>
        </div>
        """, unsafe_allow_html=True)
